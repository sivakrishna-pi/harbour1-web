...
$order_id = $json['message']['id'];

$product_details = $json['message']['order_details']['products'];

$api_total = $json['message']['order_details']['api_total'];
$api_otc_total = $json['message']['order_details']['api_otc_total'];
$api_discount_total = $json['message']['order_details']['api_discount_total'];
$api_total_tax = $json['message']['order_details']['api_total_tax'];
$api_grand_total = $json['message']['order_details']['api_grand_total'];
$grand_total = $json['message']['order_details']['grand_total'];
$promocode = $json['message']['order_details']['promocode'];
$api_grand_total = $json['message']['order_details']['api_grand_total'];
...
// echo $api_total.'<br/>';
// echo $api_otc_total.'<br/>';
// echo $api_discount_total.'<br/>';
// echo $api_total_tax.'<br/>';
// echo $api_grand_total.'<br/>';
// echo $grand_total.'<br/>';
// echo $promocode.'<br/>';
// echo $api_grand_total.'<br/>';
...
<div class="payment-ack-header">
	...
	<div class="" style="padding-bottom:10px;">
		<article class="col-md-8">
			...
			<span style="font-size:14px;">Placed on '.getproperdate(getDBdata($order_id,'orders','order_id','order_date')).'</span><br>
			<span style="font-size:14px;">Email ID: <span class="bold">'.getDBdata($order_id,'order_addresses','order_id','email').'</span></span>
		</article>
	</div><br>
	<div class="" style="padding-bottom:10px;">
...
foreach ($product_details as $key => $value) {
	...
	$body.='<td><h5 class="ack-subtitle bold">'.$value['subplan'].'</h5>';
		...
		if($value['ram'] != '0 GB')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Ram: '.$value['ram'].'</small></p>';
	    if($value['cpu'] != '0 Cores')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>CPU: '.$value['cpu'].'</small></p>';
		if($value['datatransfer'] != '0 GB')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Data Transfer: '.$value['datatransfer'].'</small></p>';
		if($value['diskspace'] != '0 GB')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Disk Space: '.$value['diskspace'].'</small></p>';
		...
		if($value['server1']>'0')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Server: '.$value['server1'].' U</small></p>';
		...
		if($value['selectedOs']!=='' && $value['selectedOs']!=='0' && $value['selectedOs']>'0')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>OS: '.$value['selectedOs'].'</small></p>';
		
 		if($value['db']!=='' && $value['db']!=='0' && $value['db']>'0'){                

                    if($value['db']=="MS SQL Enterprise*"){
                    ...
                    }else if($value['db']=="MS SQL Standard*"){
                    	...
                    	$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Database: '.$value['db'].'</small></p>';
                  	}
        }
		if($value['quantity']>'0')
			$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Quantity: '.$value['quantity'].'</small></p></td>';

			$body.='<td style="text-align:center;"><p style="font-weight:600;Margin:0;padding:0;line-height:25px;color:#12314e; text-align:right;Margin-top: -23px;">
					<span style="border-bottom:1px solid #ccc;padding:0 10px 5px 10px;"> '.$value['name'].'</span>
					</p><br><p style="color:#20641b;text-indent:14px; font-weight: bold">'.$value['notes'].' </p>
							</td>';
			
							$body.='<td style="text-align:right"><span class="bold spntotal">Rs  '.moneyFormatIndia($value['plan_price']).'</span></td>';
...
$discount_para = "";
if($api_discount_total>0){
	$discount_para = '<p style="font-weight:600;Margin:0;padding:0;line-height:25px;color:#12314e; text-align:right;Margin-top: -23px; padding-right:17px;">
	<span style="border-bottom:1px solid #ccc;padding:0 10px 5px 10px;">Discount : Rs '.moneyFormatIndia((int)$api_discount_total).'</span></p>
<br><br>';
...
<br><br>'.$discount_para.'
<p style="font-weight:600;Margin:0;padding:0;line-height:25px;color:#12314e; text-align:right;Margin-top: -23px; padding-right:17px;">
<span style="border-bottom:1px solid #ccc;padding:0 10px 5px 10px;">OTC : Rs '.$api_otc_total.'</span></p>

<br>
<br>
<p style="font-weight:600;Margin:0;padding:0;line-height:25px;color:#12314e; text-align:right;Margin-top: -23px; padding-right:17px;">
<span style="border-bottom:1px solid #ccc;padding:0 10px 5px 10px;">GST (18%): Rs '.moneyFormatIndia((int)($api_total_tax)).'</span></p>

<hr>

<div class="text-right" align="right" style="padding-right:17px;">

<b style="font-size:16px; ">Total Amount <span class="bold"> Rs  '.moneyFormatIndia($api_grand_total).'</span></b>
...
<div class="cust-info">
	...
	<span style="font-size:13px;">'.getDBdata($order_id,'order_addresses','order_id','first_name').'</span><br>

	<span style="font-size:13px;">'.getDBdata($order_id,'order_addresses','order_id','company').'</span><br>

	<span style="font-size:13px;">'.getDBdata($order_id,'order_addresses','order_id','mobile').'</span><br>

	<span style="font-size:13px;">'.getDBdata($order_id,'order_addresses','order_id','email').'</span><br>

	<span style="font-size:13px;">'.getDBdata($order_id,'order_addresses','order_id','address_line2').'</span><br>
