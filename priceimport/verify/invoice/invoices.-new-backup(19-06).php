...
foreach ($product_details as $key => $value) {
	...
	if($value['quantity']>'0')
		...
		<td>'.($value['api_monthly_price']*$value['quantity']).'</td>
	</tr>';

	$tot_mon_rec_charges = $tot_mon_rec_charges+(($value['api_monthly_price']*$value['quantity']));
	$i = $i + 1;
...
foreach ($product_details as $key => $value) {
	...
	$tot_mon_rec_charges_network = $tot_mon_rec_charges_network+$value['IpPrice'];
	$j = $j + 1;
...
foreach ($product_details as $key => $value) {
	...
	$body.='<td>'.$value['quantity'].'</td>';
	$body.='<td>'.($value['osPrice']*$value['quantity']).'</td>
	</tr>';

	$tot_mon_rec_charges_os = $tot_mon_rec_charges_os+($value['osPrice']*$value['quantity']);
	$k = $k + 1;
...
$body .='
	...
	<td style="background:#ffc000;">'.$api_otc_total.'</td>
	</tr>
	</table>
