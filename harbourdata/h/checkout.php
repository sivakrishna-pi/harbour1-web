...
<input type="hidden" name="packips[]" class="packips" value="<?php echo $packips[$q];?>">
     ...
     <?php if(($packselectedsub[$q]=='Enterprise Dedicated Servers - E3')|| ($packselectedsub[$q]=='Enterprise Dedicated Servers - E5')){
                  
                   
                 if($packmonths[$q]==1){$otc = ($packprice[$q]+$ip[$q]+$data_total[$q])*2;}
                   else if($packmonths[$q]==3){$otc = ($packprice[$q]+$ip[$q]+$data_total[$q])*2;}
                   else if($packmonths[$q]==6){$otc = ($packprice[$q]+$ip[$q]+$data_total[$q])*1.5;} 
                   else if($packmonths[$q]==12){$otc = ($packprice[$q]+$ip[$q]+$data_total[$q]);}
                   else if($packmonths[$q]==24){$otc = ($packprice[$q]+$ip[$q]+$data_total[$q]);}
                   else if($packmonths[$q]==36){$otc = ($packprice[$q]+$ip[$q]+$data_total[$q]);}
                    }else{
                      $otc = 0;
                    }

                    ?>
     <input type="hidden" name = "otc123[]" class="otc123" value="<?php echo $otc;?>">
...
<input type="checkbox" name="terms"> I have read and agree to the following <a target="_blank" href="https://pidatacenters.com/terms-conditions/" rel="nofollow" >Terms and conditions</a>
</div>
            </div>
      ...
      <tr class="grprice-rt" style=" background: #18294a; width: 100%; margin-top: 5px;" >
          ...
          <div class="row-cart">
            ...
            <div class="cart-buttons">
              ...
              <button class="bold budgfgtton" onclick="javascript:void(0)" type="submit">Pay now</button>
...
<script type="text/javascript" src="js/custom-script_v3.js?07102017"></script> 
