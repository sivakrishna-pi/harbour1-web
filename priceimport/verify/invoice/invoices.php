							...
							<table width="100%" cellpadding="3" cellspacing="0">
								...
								if(mysqli_num_rows($product_details_result) > '0'){
									...
									while($value=mysqli_fetch_array($product_details_result)){
										...
										if($value['backup'] != '0 GB')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Backup Licence Type: '.$value['backup_licence'].'</small></p>';
										$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Dedicated Bandwidth: '.$value['bandwidth'].'</small></p>
											
											<!--<p style="color: red;">
											
											<small>* The price displayed does not include backup license. Please write to us at sales@pidatacenters.com for purchasing additional backup licenses.
											</small>
											</p>-->'; 
										if($value['ip']>'0')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>IPs: '.$value['ip'].'</small></p>';
										if($value['vpn']!=null)$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>VPN: '.$value['vpn'].'</small></p>
																		<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>VPN Quantity: '.$value['vpnqty'].'</small></p>';
										...
										if($value['antivirus']!=='' && $value['antivirus']!=='0' && $value['antivirus']>'0')$body.='<p style="Margin:0;padding:0;line-height:20px;color:#262626;text-indent:14px;"><small>Antivirus: '.$value['antivirus'].'</small></p>';
										...
										if($subcribed_user){
											...
											if($value['tenure']>'0'){
												if($value['tenure']=='1'){
													...
													$grandTotalTenureText = 'Per Month';
												}
													...
													$grandTotalTenureText = 'Per Quarter';					
											...
											$body.='<td style="text-align:right"><span class="bold spntotal">INR  '.round($value['amount']).' '.$grandTotalTenureText.'</span></td>';
								...
								if(mysqli_num_rows($purchase_details_result) > '0'){
									...
									$body.='<table width="100%"><tr></tr><tr><td style="width:50%">';
...
<!-- mart main ends -->
					...
					$filename='ordersummary-'.$order_id;
					ob_end_flush();
