...
$orderalias=$_REQUEST['x'];

$q1 = "SELECT t1.order_id, t1.alias, t1.order_date, t1.order_by, t1.order_amount , t1.discount, t1.payment_mode, t1.order_status, t2.fullname, t2.designation, t2.company_name, t2.industry, t2.addresstype, t2.address, t2.city, t2.state, t2.pincode FROM lj_orders t1 INNER JOIN lj_order_address t2 ON t1.alias=t2.order_alias WHERE t1.alias='$orderalias'";

$rs1=mysqli_query($mr_con,$q1);

$esrt=mysqli_fetch_array($rs1);

$rescode=0;

$paydate=$esrt['order_date'];

$paymentID=0;

$amount=$esrt['order_amount'];
$discount=$esrt['discount'];

$merchantRefNo=$esrt['order_id'];

$billingName=$esrt['fullname'];

$billingAddress=$esrt['address'];

$billingCity=$esrt['city'];

$billingState=$esrt['state'];

$billingPostalCode=$esrt['pincode'];

$billingPhone=alias($esrt['order_by'],'lj_logins','alias','mobile_no');

$billingEmail=alias($esrt['order_by'],'lj_logins','alias','username');

$transactionID=0;

$tequestID=0;

//$orderalias=$merchantRefNo;

mysqli_query($mr_con,"UPDATE lj_orders SET payment_id='$paymentID',trans_id='$transactionID',request_id='$tequestID' WHERE alias='$orderalias'");

mysqli_query($mr_con,"DELETE FROM lj_payments_pend WHERE orderalias='$orderalias'");

$output = shell_exec('arg.sh Pi_cirros m1.tiny demo INSTANCE_NETWORK 10.0.12.0/24 True ROUTER Floating_IP Mymgs-1 demo demo True');
...
$(document).ready(function () {
    ...
    $('html, body').animate({
        scrollTop: $('#scroll-to3').offset().top -75
    }, 'slow');
});
</script>
...
<section  
    ...
    height: 100%;
    width: 100%;" id="scroll-to5">
<div id="prcing-cloud-servers2" ></div>
      <div class="header-caption">

        <div class="container" id="scroll-to4">
...
<div class="main-cart" id="scroll-to3">
  ...
  <div class="wrapper-cart transbg" id="scroll-to">
    ...
    <div class="pay-ack">
    	...
    	<div class="payment-ack-header">
        	...
        	<div class="title">
                ...
                <article>
                    ...
                    <?php if($rescode!='0'){?><p>Online payment failed. Our team will help you proceed further.</p><?php }?>
        ...
        <div class="payment-ack-body">
        	...
        	<div class="table-ack">
            	...
            	<table width="100%" cellpadding="0" cellspacing="0" id="scroll-to1">
				...
				<?php $body="";
                    ...
                    $sqlw = "SELECT  t3.plan_id, t3.ram, t3.cpu_cores,t3.data_transfer, t3.disk_space, t3.backup, t3.iops, t3.drive,t3.server1, t3.power, t3.selectedOs, t3.db,t3.months,t3.price,t3.quantity, t3.plan_amount,t3.otc FROM lj_selected_plans1 t3  WHERE t3.order_id='".$orderalias."'";
                   ...
                   // if($roww['db']=="MS SQL Enterprise*" || $roww['db']=="MS SQL Standard*"){
                    ...
                    if($roww['quantity']>'0')$body.='<li>Quantity : '.$roww['quantity'].' </li>';
                    //if($roww['rackspace']>'1')$body.='<li>Rackspace '.$roww['rackspace'].' Units</li>';
                  ...
                  //  $body.='<td valign="middle" style="vertical-align:middle;">Rs '.round($roww['plan_amount']/$roww['months'],2).'</td>';
                    ...
                    if($roww['months']>'0')if($roww['months']=='1'){$tenure= "Monthly";}
                      ...
                      //$body.='<li>Tenure : '.$tenure.' </li>';

                      $body.='<td valign="middle" style="vertical-align:middle; text-align: center; text-align: center;">'.$tenure.' <br>
                    <br>';
