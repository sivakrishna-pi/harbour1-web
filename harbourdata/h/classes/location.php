...
class location extends dbconfig {
 ...
 // Fetch all countries list
   public static function getCountries() {
     try {
       $query = "SELECT id, name FROM countries_v2";
  ...
  public static function getStates($countryId) {
     try {
       $query = "SELECT id, name FROM states_v2 WHERE country_id=".$countryId;
       $result = dbconfig::run($query);
       if(!$result) {
 ...
 // Fetch all cities list by state id
  public static function getCities($stateId) {
     try {
       $query = "SELECT id, name FROM cities_v2 WHERE state_id=".$stateId;
       $result = dbconfig::run($query);
       if(!$result) {
